...
function Dashboard() {
  const { isAdmin, hasPermission, profile, user } = useAuth(); const { data: clients = [] } = useClients();
  const canViewDashboard = isAdmin || hasPermission("dashboard");
  const { data: documents = [], isLoading } = useQuery({ queryKey: ["dashboard-client-documents"], enabled: canViewDashboard, queryFn: async () => { const { data, error } = await (supabase.from("client_documents") as any).select("id,client_id,branch_id,title,file_name,expires_at,active").eq("active", true); if (error) throw error; return (data ?? []) as Doc[]; } });
  const { data: branches = [] } = useQuery({ queryKey: ["dashboard-client-branches"], enabled: canViewDashboard, queryFn: async () => { const { data, error } = await (supabase.from("client_branches") as any).select("id,name,cnpj"); if (error) throw error; return (data ?? []) as Branch[]; } });
