...
function UsersPage() {
  ...
  const [credentialsTarget, setCredentialsTarget] = useState<any | null>(null);
  ...
  const updateClientLogin = useMutation({
    mutationFn: async () => {
      if (!credentialsTarget?.id) throw new Error("Cliente inválido.");
      if (!/^\S+@\S+\.\S+$/.test(clientLogin.trim())) throw new Error("Informe um e-mail válido.");
      const { error } = await (supabase.rpc("admin_update_client_email", {
        target_user_id: credentialsTarget.id,
        new_email: clientLogin.trim().toLowerCase(),
      }) as any);
      if (error) throw error;
    },
    onSuccess: () => {
      ...
      setClientLogin("");
      refresh();
      toast.success("Login do cliente atualizado.");
    },
    onError: (error: any) => toast.error(error?.message ?? "Não foi possível atualizar o login."),
  });
